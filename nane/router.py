...
#  KROK 3: ROUTY 
@app.route("/books", methods=["GET","POST"])  # 🟡 ZDE ZMĚŇ
def collection():
    return forward("/books")   # 🟡 ZDE ZMĚŇ

# Povoleny všechny metody: GET, PUT, PATCH i DELETE
@app.route("/books/<int:id>", methods=["GET","PUT","PATCH","DELETE"])  # 🟡 ZDE ZMĚŇ
def single(id):
    return forward(f"/books/{id}")   # 🟡 ZDE ZMĚŇ
